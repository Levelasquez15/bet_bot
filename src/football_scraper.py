#!/usr/bin/env python3
"""
Football Data Scraper - Using LanusStats Library (with fallback)
Advanced football data scraping from multiple sources with robust fallback
"""
...
# Try to import LanusStats, fallback to The Sports DB if not available
try:
    import LanusStats as ls
    LANUS_AVAILABLE = True
    logger = logging.getLogger(__name__)
    logger.info("LanusStats available - using advanced scraping capabilities")
except ImportError as e:
    logger = logging.getLogger(__name__)
    logger.warning(f"LanusStats not available: {e}. Using fallback scraper.")
    LANUS_AVAILABLE = False
    ls = None

class FootballAPIScraper:
    """Scraper using LanusStats library with fallback to The Sports DB"""

    def __init__(self):
        if LANUS_AVAILABLE:
            # Initialize different sources
            try:
                self.fbref = ls.Fbref()
                self.fotmob = ls.FotMob()
                self.sofascore = ls.SofaScore()
                self.threesixfivescores = ls.ThreeSixFiveScores()
                logger.info("All LanusStats sources initialized successfully")
    ...
    def _init_fallback(self):
        """Initialize fallback scraper using The Sports DB API"""
        import requests
    ...
    def _get_upcoming_lanus(self, league_name: str, days_ahead: int) -> pd.DataFrame:
        ...
        try:
            ...
            # For now, return fallback since FotMob tables don't give direct fixtures
            return self._get_upcoming_fallback(league_name, days_ahead)
